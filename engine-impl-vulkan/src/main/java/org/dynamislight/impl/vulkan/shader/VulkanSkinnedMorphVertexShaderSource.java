...
public final class VulkanSkinnedMorphVertexShaderSource {
    private VulkanSkinnedMorphVertexShaderSource() {
    }
    ...
    public static String mainVertex() {
        return """
                ...
                layout(set = 2, binding = 2) readonly buffer JointMatrices {
                    mat4 joints[];
                } skeleton;
                layout(set = 3, binding = 3) readonly buffer MorphDeltas {
                    float deltas[];
                } morphBuffer;
                layout(set = 3, binding = 4) uniform MorphWeights {
                    float weights[256];
                } morphWeights;
                ...
                void main() {
                    ...
                    for (int t = 0; t < morphTargetCount; t++) {
                        ...
                        float w = morphWeights.weights[t];
                        morphedPos += w * vec3(
                            morphBuffer.deltas[base],
                            morphBuffer.deltas[base + 1],
                            morphBuffer.deltas[base + 2]
                        );
                        morphedNormal += w * vec3(
                            morphBuffer.deltas[base + 3],
                            morphBuffer.deltas[base + 4],
                            morphBuffer.deltas[base + 5]
                        );
                    }
                    ...
                    vHeight = worldPos.y;
                    vNormal = normalize(mat3(obj.uModel) * skinRot * morphedNormal);
