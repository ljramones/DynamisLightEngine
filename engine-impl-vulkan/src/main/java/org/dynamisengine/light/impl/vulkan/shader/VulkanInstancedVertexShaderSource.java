...
public final class VulkanInstancedVertexShaderSource {
    private VulkanInstancedVertexShaderSource() {
    }
    ...
    public static String mainVertex() {
        return """
                ...
                layout(set = 2, binding = 5) readonly buffer InstanceData {
                    vec4 raw[];
                } instances;
                layout(push_constant) uniform MainPush {
                    vec4 uPlanar;
                } pc;
                mat4 instanceModel(uint index) {
                    int base = int(index) * 5;
                    return mat4(
                        instances.raw[base + 0],
                        instances.raw[base + 1],
                        instances.raw[base + 2],
                        instances.raw[base + 3]
                    );
                }
                void main() {
                    mat4 model = instanceModel(gl_InstanceIndex);
