...
public final class VulkanBindlessSkinnedVertexShaderSource {
    private VulkanBindlessSkinnedVertexShaderSource() {
    }
    ...
    public static String mainVertex() {
        return """
                ...
                layout(set = 3, binding = 0) readonly buffer JointPalette {
                    mat4 joints[];
                } jointHeap[];
                ...
                void main() {
                    ...
                    mat4 skinMatrix =
                        inWeights.x * jointHeap[nonuniformEXT(jointIdx)].joints[inJoints.x] +
                        inWeights.y * jointHeap[nonuniformEXT(jointIdx)].joints[inJoints.y] +
                        inWeights.z * jointHeap[nonuniformEXT(jointIdx)].joints[inJoints.z] +
                        inWeights.w * jointHeap[nonuniformEXT(jointIdx)].joints[inJoints.w];
                    mat3 skinRot = mat3(skinMatrix);
                    vec4 localPos = skinMatrix * vec4(inPos, 1.0);
                    vec4 worldPos = obj.uModel * localPos;
                    bool planarCapturePass = pc.uPlanar.x > 0.5;
                    vWorldPos = worldPos.xyz;
                    vHeight = worldPos.y;
                    vNormal = normalize(mat3(obj.uModel) * skinRot * inNormal);
                    vTangent = normalize(mat3(obj.uModel) * skinRot * inTangent);
                    vUv = inUv;
