...
- GitHub Actions `shadow-phased-lockdown` runs on `push`/`pull_request`/`schedule` and enforces transparent/area/distance policy + Phase D promotion stability gates.
...
- `shadow-lockdown-full` workflow lane now runs on `push`/`pull_request`/`schedule` (plus optional manual dispatch). It executes:
...
- Local strict Phase D gate runner:
  - `./scripts/shadow_phased_lockdown.sh`
