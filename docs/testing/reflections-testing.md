...
- Assert dedicated RT preview activation emits `REFLECTION_RT_DEDICATED_PIPELINE_ACTIVE` in mock contract lane when `vulkan.reflections.rtDedicatedPipelineEnabled=true`.
...
9. Per-material override envelope checks
- Assert `REFLECTION_OVERRIDE_POLICY` and `REFLECTION_OVERRIDE_POLICY_ENVELOPE` are emitted for mixed-override scenes.
