...
- Scene-level post-process DTO now available:
  - `PostProcessDesc` on `SceneDescriptor` (tonemap + bloom + SSAO controls, including radius/bias/power shaping)
...
- TAA reprojection maturity:
  ...
  - Debug output modes are available via backend options (`opengl.taaDebugView` / `vulkan.taaDebugView`: `0=off`, `1=reactive`, `2=disocclusion`, `3=historyWeight`, `4=velocity`, `5=compositeOverlay`).
  - Scene-level AA DTO is available through `PostProcessDesc.antiAliasing` (`AntiAliasingDesc`) for host-driven temporal control overrides.
...
- Guarded compare-harness includes `post-process`, `post-process-bloom`, `post-process-ssao`, `post-process-ssao-stress`, and `post-process-smaa` profiles.
...
- Tiered golden envelopes now include `post-process`, `post-process-bloom`, `post-process-ssao`, `post-process-ssao-stress`, and `post-process-smaa` (`LOW/MEDIUM/HIGH/ULTRA`).
...
- Current ULTRA `post-process-ssao-stress` bound: `<= 0.37`.
