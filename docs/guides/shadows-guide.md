...
- Shadow quality-path requests are now first-class backend options:
  ...
  Advanced quality tuning is now runtime-configurable via backend options (`pcssSoftness`, `momentBlend`, `momentBleedReduction`, `contactStrength`) without shader edits. Current Vulkan shading path also applies neighborhood-weighted plus edge-aware moment denoise, a deep wide bilateral consistency pass, moment-informed PCSS blocker estimation (when moment data is available), a dedicated BVH-mode traversal approximation function (`bvhTraversalVisibilityApprox`) with RT denoise refinement, and motion-adaptive contact stabilization to reduce high-motion flicker and edge bleeding.
