...
| Shadows | Cascaded directional shadows, multi-local point/spot lighting (current cap `8`), shadow-policy local-light budgets (`1/2/3/4` by tier), per-type normal/slope bias scaling, PCF + tier controls, OpenGL local spot atlas render/sampling path with cadence/cache reuse, Vulkan multi-local spot layers + tier-bounded concurrent point cubemap rendering (`HIGH`: 1, `ULTRA`: 2, override headroom to 24 faces/layers) with local point layer-base to cubemap-face matrix generation, scheduler overrides (`vulkan.shadow.maxShadowedLocalLights`, `vulkan.shadow.maxLocalShadowLayers`, `vulkan.shadow.maxShadowFacesPerFrame`), cadence controls (`vulkan.shadow.scheduler.*`), runtime directional cascade texel-snap controls (`vulkan.shadow.directionalTexelSnapEnabled`, `vulkan.shadow.directionalTexelSnapScale`), age/priority ordering telemetry (`renderedShadowLightIds`), backlog telemetry (`deferredShadowLightCount`, `deferredShadowLightIds`), and moment-atlas budget telemetry (`shadowMomentAtlasBytesEstimate`) with explicit requested-vs-active filter reporting (`filterPath` vs `runtimeFilterPath`, `momentFilterEstimateOnly`) and moment-pipeline/resource state reporting (`momentPipelineRequested`, `momentPipelineActive`, `momentResourceAllocated`, `momentResourceFormat`, `momentInitialized`, `momentPhase`) plus provisional active-path warning (`SHADOW_MOMENT_APPROX_ACTIVE`) and quality-path request controls for `pcss`/`vsm`/`evsm`/contact/RT modes | Advanced shadow stack | VSM + RT paths | Cascaded + RT options | Clustered shadows + CSM |
...
- Remaining shadow gap is explicit: full Vulkan per-light atlas/cubemap parity is still rolling out, and true production VSM/EVSM/PCSS/contact/RT shadow paths are still pending (current runtime is PCF/PCSS active with `vsm/evsm` estimate-only tracking).
