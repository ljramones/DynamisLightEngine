...
| Shadows | Cascaded directional shadows, multi-local point/spot lighting (current cap `8`), shadow-policy local-light budgets (`1/2/3/4` by tier), per-type normal/slope bias scaling, PCF + tier controls, OpenGL local spot atlas render/sampling path with cadence/cache reuse, Vulkan multi-local spot layers + concurrent point cubemap scheduling (policy/layer bounded; up to 2 point cubemaps on higher tiers), plus quality-path request controls for `pcss`/`vsm`/`evsm`/contact/RT modes | Advanced shadow stack | VSM + RT paths | Cascaded + RT options | Clustered shadows + CSM |
