...
| Shadows | Cascaded directional shadows, multi-local point/spot lighting (current cap `8`), shadow-policy local-light budgets (`1/2/3/4` by tier), per-type normal/slope bias scaling, PCF + tier controls, OpenGL local spot atlas render/sampling path with cadence/cache reuse, Vulkan multi-local spot shadow-layer rendering (within current layer budget), plus quality-path request tracking for `pcss`/`vsm`/`evsm`/contact/RT modes (production filter still PCF fallback) | Advanced shadow stack | VSM + RT paths | Cascaded + RT options | Clustered shadows + CSM |
...
- Remaining shadow gap is explicit: full Vulkan per-light atlas/cubemap parity is still rolling out, with the biggest outstanding item being multi-point cubemap shadow concurrency and dedicated VSM/EVSM/PCSS/RT production paths.
