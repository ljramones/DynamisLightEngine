...
| PBR/Shading | PBR-leaning GGX baseline, BRDF LUT + IBL hooks, tier-aware degradation | Full PBS/PBR | Full PBR | Full PBR | Full PBR |
...
| Post-Processing | Tonemap + bloom baseline (OpenGL FBO chain, Vulkan offscreen path) | Mature cinematic stack | Extensive post suite | Extensive HDRP post suite | Compositor-driven post |
...
- Strong regression harness (`--compare`, tiered golden thresholds, stress profiles).
