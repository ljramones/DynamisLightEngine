...
- Remaining shadow gap is explicit: full Vulkan per-light atlas/cubemap parity is still rolling out, and true production VSM/EVSM/PCSS/contact/RT shadow paths are still pending (current runtime is PCF/PCSS active with `vsm/evsm` estimate-only tracking). Multi-point cubemap concurrency is now landed within current tier/override scheduler budgets.
