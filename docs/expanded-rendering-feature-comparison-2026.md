...
| Anti-Aliasing | SMAA-lite + jitter-aware TAA temporal-history blend with global camera-motion reprojection in both OpenGL and Vulkan post paths (full per-pixel motion-vector-grade reprojection still pending) | TAA | TSR/TAA | TAAU (+ upscalers) | TAA/MSAA |
...
- Baseline TAA controls are now wired in both backends, improving post-process parity tracking.
