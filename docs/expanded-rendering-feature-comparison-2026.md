...
| Shadows | Cascaded directional shadows, multi-local point/spot lighting (current cap `8`), shadow-policy local-light budgets (`1/2/3/4` by tier), per-type normal/slope bias scaling, PCF + tier controls, OpenGL local spot atlas render/sampling path with cadence/cache reuse, Vulkan primary-local shadow render baseline (explicit warning + CI tracking while atlas/cubemap parity rolls out) | Advanced shadow stack | VSM + RT paths | Cascaded + RT options | Clustered shadows + CSM |
...
- Remaining shadow gap is explicit: Vulkan primary-local shadow rendering is still the active baseline; full per-light local atlas plus cubemap shadow rendering is still rolling out (tracked via `SHADOW_LOCAL_RENDER_BASELINE` and shadow CI matrix).
